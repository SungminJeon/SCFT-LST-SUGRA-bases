#!/usr/bin/env python3
"""For each SJ-only entry (in our catalog, not in Hamada at T_H=1..7), compute
the fiber-weighted true c_ext and split by ≤16 vs >16."""
import csv, glob, os, re, sys
csv.field_size_limit(sys.maxsize)
import numpy as np
BASE = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

# --- content_key (mirror compare_delta_T7) ---
def gauge_content(s):
    s = (s or "").strip()
...
# --- true c_ext (mirror check_true_cext.py) ---
GAUGE_DIM_H = {
...
def find_kernel_positive_integer(A):
    ...
    ker_vecs = Vt[s < tol]
    if len(ker_vecs) == 0:
        # rank-deficient detection: smallest singular value
...
def per_curve_gauge_from_nhc(M, curves):
    """Compute per-curve gauge after NHC detection."""
    out = {}
    nc = len(curves)
    # Build adjacency among non-(-1) curves
    ...
    for c in non1:
        ...
        if len(cluster) == 1:
            ...
            # Could be -2-3-2 (so7+su2+su2) or -2-2-3 (g2+su2+none)
            # Distinguish by degree of -3
            m3 = [x for x in cluster if M[x][x] == -3][0]
            deg = sum(1 for y in cluster if M[m3][y] != 0 and y != m3)
            if deg == 2:  # -3 in middle → -2-3-2
                out[m3] = "so7"
                for x in cluster:
                    if M[x][x] == -2: out[x] = "su2"
            else:  # -3 at end → -2-2-3
                out[m3] = "g2"
                # mid -2 (adjacent to -3): su2; outer -2: none
...
# --- replicate compare's SJ key + grab entry IF ---
...
# Collect Hamada YH keys for T_H ∈ T_RANGE
yh_keys = set()
...
# Walk SJ entries, accumulate one representative per SJ-only key
sj_repr = {}  # key -> entry (first occurrence)
total_sj = 0
    ...
    for cf in os.listdir(os.path.join(root, dname)):
        ...
        try:
            for entry in parse_cat_file(os.path.join(root, dname, cf)):
                ...
                sj_repr[key] = (entry, root, dname, cf)
        except Exception:
            pass

print(f"Total SJ entries scanned: {total_sj}")
print(f"SJ-only unique keys: {len(sj_repr)}")
print()

# For each SJ-only, compute true c_ext
le16 = 0; gt16 = 0; fail = 0
le16_list = []
for key, (entry, root, dname, cf) in sj_repr.items():
    M = entry.get("if_matrix")
    if M is None: fail += 1; continue
    ext_idx = set()
    for e in entry["externals"]:
        if e.get("curve_idx") is not None:
            ext_idx.add(e["curve_idx"])
    if not ext_idx and "attach_if_size" in entry:
        ext_idx.add(len(M) - 1)
    n = len(M)
    lst_idx = [i for i in range(n) if i not in ext_idx]
    ext_idx = sorted(ext_idx)
    cval = compute_true_cext(M, ext_idx, lst_idx)
    if cval is None: fail += 1; continue
    if cval > 16 + 1e-6:
        gt16 += 1
    else:
        le16 += 1
        le16_list.append((key, cval, dname, cf))

print(f"true c_ext > 16: {gt16}")
print(f"true c_ext ≤ 16: {le16}")
print(f"kernel/compute failed: {fail}")
print()

if le16_list:
    print(f"## SJ-only entries with true c_ext ≤ 16 ({len(le16_list)}):")
    le16_list.sort(key=lambda r: (r[0][0], r[1]))
    for (T_H, lst, sg, d), cval, dname, cf in le16_list[:40]:
        print(f"  T_H={T_H} Δ={d:>5}  c_true={cval:.2f}  LST='{lst[:30]}'  SUGRA='{sg[:50]}'  ({dname}/{cf})")
    if len(le16_list) > 40:
        print(f"  ... +{len(le16_list)-40} more")
